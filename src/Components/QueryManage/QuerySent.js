...
const QuerySent = () => {
  const [queryData, setQueryData] = useState()
    ...
    useEffect(() => {
      // setLoading(true);
      axios.get(`http://52.66.198.155/api/v1/admin/query/${querysent}`).then((res) => {
...
<div className="border px-12 rounded-md mt-6 py-6">
      <from onSubmit={handleSentMessage} className=" ">
        ...
        <input onChange={handleMessage} name="to" type="text" placeholder="Joshep example@gmail.com" className="input input-bordered w-full" />

        <h2 className="mt-6">Subject </h2>
        <input onChange={handleMessage} name="subject" type="text" placeholder="Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nunc vulputate libero" className="input input-bordered w-full" />

        <h2 className="mt-6 text-black">Message</h2>
        <textarea onChange={handleMessage} name="message" className="textarea textarea-bordered w-full mt-1" placeholder="Bio"></textarea>

        <input onClick={handleSentMessage} type="submit" value="Sent" className="bg-primary px-6 py-1 rounded-lg text-white flex mx-auto mt-6" />
      </from>
      </div>
    </div>
  );
